...
class Main:
    def __init__(self):
        ...
        if self.MOVIEID:
            ...
            log('SKINWIDGET found EPISODEID')
            ...
            log('SKINWIDGET init clearProperty success')
    ...
    def _fetch_info_recommended(self):
        ...
        if __addon__.getSetting("recommended_enable") == 'true':
            ...
            c = b - a
            log('SKINWIDGET Total time needed to request recommended queries: %s' % c)

    def _fetch_info_randomitems(self):
        log('SKINWIDGET_fetch_info_randomitems')
        a = datetime.datetime.now()
        if __addon__.getSetting("randomitems_enable") == 'true':
            ...
            self._fetch_phrase('RandomPhrase')
            
            b = datetime.datetime.now()
            c = b - a
            log('SKINWIDGET Total time needed to request random queries: %s' % c)

    def _fetch_phrase (self, request):
        if not self.Monitor.abortRequested():    
            Phrases = __addon__.getSetting('randomphrases').split("|")
            
            count = 0
            for count in range(int(self.LIMIT)):
                count += 1
                Phrase = random.choice(Phrases)
                self.WINDOW.setProperty("%s.%d.Phrase"           % (request, count), Phrase)
    ...
    def _fetch_info_recentitems(self):
        ...
        if __addon__.getSetting("recentitems_enable") == 'true':
            ...
            b = datetime.datetime.now()
            c = b - a
            log('SKINWIDGET Total time needed to request recent items queries: %s' % c)
            
    def _fetch_movies(self, request):
        log('SKINWIDGET fetch_movies')
        if not self.Monitor.abortRequested():
            ...
            if request == 'RecommendedMovie':
                ...
                log('SKINWIDGET fetch_movies Random not just unplayed')
                json_query = xbmc.executeJSONRPC('%s "sort": {"method": "random" } }}' %json_string)
            
            log('SKINWIDGET json_query= ' + str(json_query))
    ...
    def _fetch_tvshows_recommended(self, request):
        if not self.Monitor.abortRequested():
            ...
            log('SKINWIDGET finding first unplayed episode')
            ...
            if 'result' in json_query and 'tvshows' in json_query['result']:
                log('SKINWIDGET passses result in query')
                self._clear_properties(request)
                count = 0
                for item in json_query['result']['tvshows']:
                    if self.Monitor.abortRequested():
                        break
                    count += 1
                    log('SKINWIDGET defining json_query2')
                    ...
                    if 'result' in json_query2 and json_query2['result'] != None and 'episodes' in json_query2['result']:
                        for item2 in json_query2['result']['episodes']:
                            log('SKINWIDGET for item2:' +str(item2))
                        ...
                        if item2['playcount'] >= 1:
                            ...
                            watched = "false"
                        if not self.PLOT_ENABLE and watched == "false":
                            log('SKINWIDGET PLOT_ENABLE watched false')
                            plot = __localize__(32014)
                            log('SKINWIDGET plot: ' + str(plot))
                        else:
                            log('SKINWIDGET plot else')
                            plot = item2['plot']
                            plot = plot.encode(encoding="utf-8",errors="ignore")
                            log('SKINWIDGET plot: ' + str(plot))
                        art = item['art']
                        path = media_path(item['file'])
                        ...
                        streaminfo = media_streamdetails(item['file'].lower(),
                                                         item2['streamdetails'])
                        log('SKINWIDGET about to check studio')
                        if len(item['studio']) > 0:
                            studio = item['studio'][0]
                        else:
                            studio = ""
                        
                        log('SKINWIDGET setting recommended episode properties')
    ...
    def _fetch_tvshows(self, request):
        ...
        if not self.Monitor.abortRequested():
            ...
            if request == 'RecentEpisode' and self.RECENTITEMS_UNPLAYED:
                ...
                log('SKINWIDGET random TV not just unplayed')
                json_query = xbmc.executeJSONRPC('%s "sort": {"method": "random" }}}' %json_string)
            log('SKINWIDGET json_query= ' + str(json_query))
            json_query = simplejson.loads(json_query)
...
class Widgets_Monitor(xbmc.Monitor):
    ...
    # onDatabaseUpdated Deprecated. Use onScanFinished().  
...
if (__name__ == "__main__"):
    log('SKINWIDGET script version %s started' % __addonversion__)
    Main()
    del Widgets_Monitor
    del Widgets_Player
    del Main
    log('SKINWIDGET script version %s stopped' % __addonversion__)
